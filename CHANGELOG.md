...
<a id="en-v2800-summary"></a>

### Summary

**Data-layer refactor.** v2.8.0.0 lifts all 151 policy definitions out of the script body into a validated, machine-readable data layer: `Brave Omega/config.json` (registry targets + level order) and `Brave Omega/profiles/*.json` (one file per tier). `BraveOmega.ps1` now loads policies at runtime through `Import-OmegaPolicyData` into a single `$OmegaState`, producing byte-identical registry output. The Pester suite, the ADMX cross-reference validator, and the policy-catalog generator read the same data layer, making it the single source of truth for build, test, and docs. No policies changed: totals remain **151** across 5 tiers; cumulative chain remains 24 → 51 → 83 → 123 → 151.
...
<a id="en-v2800-changed"></a>

### Changed

- **BraveOmega.ps1** - `$ScriptVersion` → `v2.8.0.0`; policy definitions moved out of the script into the data layer; runtime loads policies via `Import-OmegaPolicyData` (single `$OmegaState`).
- **Data layer** - new `Brave Omega/config.json` + `Brave Omega/profiles/<Tier>.json` with 151 policies is now the single source of truth; ADMX validator, Pester suite, and catalog generator all read the same data.
- **Tests & CI** - TestHelper and tests re-pointed to the data layer; Pester job in `quality.yml` now also emits a CodeCoverage report (informational).
- **Version matrices and catalog** - README, SECURITY, index.html, Wiki compatibility matrix, and regenerated `levels.json` bumped; `.reg` artifacts byte-identical (no policy drift).
...
<a id="en-v2730-summary"></a>

### Summary

**Brave 1.95.104 compatibility validation.** v2.7.3.0 validates the configuration against Brave 1.95.104 (Chromium 153.0.8010.53), released September 18, 2026. Brave 1.95.102 (Chromium 153.0.8010.48) remains supported. No policies changed: totals remain **151** across 5 tiers; cumulative chain remains 24 → 51 → 83 → 123 → 151.
...
| Total policies | 151 | **151** (no change) |
| Cumulative chain | 24→51→83→123→151 | **24→51→83→123→151** |
| Script version | v2.7.2.0 | v2.7.3.0 |
| Validated Brave | 1.95.102 | 1.95.104 |
| Validated Chromium | 153.0.8010.48 | 153.0.8010.53 |

<a id="en-v2730-changed"></a>

### Changed

- **BraveOmega.ps1** — `$ScriptVersion` → `v2.7.3.0`; `$ValidatedBrave` → `1.95.104`; `$ValidatedChromium` → `153`; header changelog and channel warning updated.
- **Version matrices and catalog** — README, SECURITY, index.html, Wiki compatibility matrix, and regenerated `levels.json` bumped; `.reg` artifacts byte-identical (no policy drift).
...
<a id="en-v2720-summary"></a>

### Summary

**Brave 1.95.102 compatibility validation.** v2.7.2.0 validates the configuration against Brave 1.95.102 (Chromium 153.0.8010.48), released September 16, 2026. Brave 1.95.101 (Chromium 153.0.8010.37) remains supported. No policies changed: totals remain **151** across 5 tiers; cumulative chain remains 24 → 51 → 83 → 123 → 151.
...
| Total policies | 151 | **151** (no change) |
| Cumulative chain | 24→51→83→123→151 | **24→51→83→123→151** |
| Script version | v2.7.1.0 | v2.7.2.0 |
| Validated Brave | 1.95.101 | 1.95.102 |
| Validated Chromium | 153.0.8010.37 | 153.0.8010.48 |

<a id="en-v2720-changed"></a>

### Changed

- **BraveOmega.ps1** — `$ScriptVersion` → `v2.7.2.0`; `$ValidatedBrave` → `1.95.102`; `$ValidatedChromium` → `153`; header changelog and channel warning updated.
- **Version matrices and catalog** — README, SECURITY, index.html, Wiki compatibility matrix, and regenerated `levels.json` bumped; `.reg` artifacts byte-identical (no policy drift).
...
<a id="en-v2710-summary"></a>

### Summary

**Brave 1.95.101 compatibility validation.** v2.7.1.0 validates the configuration against Brave 1.95.101 (Chromium 153.0.8010.37), released September 11, 2026. Brave 1.94.121 (Chromium 152.0.7977.83) remains supported. No policies changed: totals remain **151** across 5 tiers; cumulative chain remains 24 → 51 → 83 → 123 → 151.
...
| Total policies | 151 | **151** (no change) |
| Cumulative chain | 24→51→83→123→151 | **24→51→83→123→151** |
| Script version | v2.7.0.0 | v2.7.1.0 |
| Validated Brave | 1.94.121 | 1.95.101 |
| Validated Chromium | 152.0.7977.83 | 153.0.8010.37 |

<a id="en-v2710-changed"></a>

### Changed

- **BraveOmega.ps1** — `$ScriptVersion` → `v2.7.1.0`; `$ValidatedBrave` → `1.95.101`; `$ValidatedChromium` → `153`; header changelog and channel warning updated.
- **Version matrices and catalog** — README, SECURITY, index.html, Wiki compatibility matrix, and regenerated `levels.json` bumped; `.reg` artifacts byte-identical (no policy drift).
...
<a id="en-v2700-summary"></a>

### Summary

**Unified bilingual script.** v2.7.0.0 merges `BraveOmega-EN.ps1` and `BraveOmega-TR.ps1` into a single `BraveOmega.ps1` with a 108-key EN/TR strings table: first launch asks `Press 1 for English / Türkçe için 2'ye basın`, or pin the language with `-Language EN|TR|Auto`. `Brave Omega/` now holds exactly one script. No policies changed: totals remain **151** across 5 tiers; cumulative chain remains 24 → 51 → 83 → 123 → 151. Validated Brave stays 1.94.121 (Chromium 152.0.7977.83).

| Metric | Before (v2.6.2.0) | After (v2.7.0.0) |
|--------|-------------------|-------------------|
| Script files | 2 (EN + TR) | **1** (unified bilingual) |
| UI languages | 2 files | **2** (one strings table) |
| Total policies | 151 | **151** (no change) |
| Cumulative chain | 24→51→83→123→151 | **24→51→83→123→151** |
| Script version | v2.6.2.0 | v2.7.0.0 |
| Validated Brave | 1.94.121 | 1.94.121 |
| Validated Chromium | 152.0.7977.83 | 152.0.7977.83 |

<a id="en-v2700-added"></a>

### Added

- **Unified bilingual script** — `BraveOmega.ps1` carries all UI text in a `$Strings` table (108 keys, EN + TR) plus `$LevelDisplayNames`; `-Language EN|TR|Auto` with a bilingual startup prompt; TR aliases kept (`-Seviye`, `-Sifirla`, `-SenkronizasyonaIzinVer`, `-Dil`).
- **Gitleaks secret-scan CI** — pinned v8.30.1 binary with SHA-256 verification scans the full history on push and pull requests.

<a id="en-v2700-changed"></a>

### Changed

- **Single script under `Brave Omega/`** — the EN/TR wrappers are removed; tests, ADMX validator, workflows, catalog tooling, README, index.html, Wiki, and templates updated to the wrapper-free layout.
- **Fixed latent TR bug** — the S/MIME install notice never displayed in Turkish runs (Turkish display name compared against English level keys); canonical `$Level` fixes it, covered by a regression test.
...
<a id="en-v2610-removed"></a>

### Removed

- **`DeviceAttributesAllowedForOrigins`** — Removed from the Essential tier. ChromeOS-only Device Attributes API policy unsupported by Brave on Windows (runtime enforcement returns an unknown-policy error); no Windows hardening impact.

<a id="en-v2610-changed"></a>
...
- **Documentation & tests** — README, Wiki, SECURITY.md, index.html and the Pester test suite updated to 151 policies.
...
- **Tier counts** — Advanced 38 › **40**, total 150 › **152**; cumulative chain now 24›52›84›124›152 (BraveOnly 24, Essential 28, Balanced 32, Advanced 40, Strict 28).
- **BraveOmega-EN.ps1 / BraveOmega-TR.ps1** — `$ScriptVersion`/`$BetikSurum` › `v2.6.0.0`, header and changelog updated.
- **Documentation & tests** — policy-catalog, Brave-Group-Policy-Reference, Wiki, README, SECURITY.md, index.html, and the Pester test suite updated to 152 policies.
...
<a id="en-v2554-changed"></a>

### Changed

- **Brave version bump** — Validated against Brave **1.94.117** (Chromium 152.0.7977.64). Brave 1.93.138 (Chromium 151.0.7922.173), Brave 1.93.136 (Chromium 151.0.7922.137) and Brave 1.93.129 (Chromium 151.0.7922.71) remain supported.
- **BraveOmega-EN.ps1 / BraveOmega-TR.ps1** — `$ScriptVersion`/`$BetikSurum` › `v2.5.5.4`, header + changelog updated.
- **Documentation & tests** — Version references refreshed across README, SECURITY.md, index.html, Wiki, release notes and the Pester test suite.
...
| Validated Chromium | 151.0.7922.137 | 151.0.7922.173 |

<a id="en-v2553-changed"></a>

### Changed

- **Brave version bump** — Validated against Brave **1.93.138** (Chromium 151.0.7922.173). Brave 1.93.136 (Chromium 151.0.7922.137) remains supported.
- **BraveOmega-EN.ps1 / BraveOmega-TR.ps1** — `$ScriptVersion`/`$BetikSurum` › `v2.5.5.3`, header + changelog updated.
- **Documentation & tests** — Version references refreshed across README, SECURITY.md, index.html, Wiki, release notes and the Pester test suite.
...
| Validated Chromium | 151.0.7922.71 | 151.0.7922.137 |

<a id="en-v2552-fixed"></a>

### Fixed

- **Critical Reset-mode regression (EN + TR)** — `$HKCU_Target`/`$HKLM_Target` and `$HKCU_Hedef`/`$HKLM_Hedef` path constants were defined **after** the `-Reset`/`-Sifirla` block. Reset mode therefore evaluated `Test-Path $null` (PowerShell 5.1 throws `ParameterBindingValidationException`) and crashed before removing any policies. Definitions moved above the Reset block; duplicate late definitions removed.
- **ADMX validator hardcoded map** — `admx/admx-validate.ps1` validated only ~80 hardcoded policy names. It now auto-discovers all 150 policies directly from `BraveOmega-EN.ps1` and validates every one (type + ValueName) against `brave.admx`.

<a id="en-v2552-changed"></a>

### Changed

- **Brave version bump** — Validated against Brave **1.93.136** (Chromium 151.0.7922.137). Brave 1.93.129 (Chromium 151.0.7922.71) remains supported.
- **ADMX validator documented exceptions** — `$knownAdmxExceptions` allows policies intentionally absent from Brave's ADMX to be reported as warnings instead of hard failures (only `DeviceAttributesAllowedForOrigins` listed, documented in CHANGELOG; do not use it to silence typos).
- **BraveOmega-EN.ps1 / BraveOmega-TR.ps1** — `$ScriptVersion`/`$BetikSurum` › `v2.5.5.2`, header + changelog updated.
- **Tests** — new regression tests: Reset-mode path-constant ordering (EN/TR), ADMX validator auto-discovery count, and documented-exception handling.
...
|---------|------------|----------|---------------|
| v2.8.0.0 | 2026-09-22 | 151   | Data-layer refactor: policy definitions moved to config.json + profiles/*.json; script loads via Import-OmegaPolicyData; runtime behavior unchanged; total 151; cumulative chain 24→51→83→123→151 |
| v2.7.3.0 | 2026-09-19 | 151   | Compatibility validation: Brave 1.95.104 (Chromium 153.0.8010.53); no policy changes; total 151; cumulative chain 24→51→83→123→151 |
| v2.7.2.0 | 2026-09-18 | 151   | Compatibility validation: Brave 1.95.102 (Chromium 153.0.8010.48); no policy changes; total 151; cumulative chain 24→51→83→123→151 |
| v2.7.1.0 | 2026-09-11 | 151   | Compatibility validation: Brave 1.95.101 (Chromium 153.0.8010.37); no policy changes; total 151; cumulative chain 24→51→83→123→151 |
| v2.7.0.0 | 2026-09-09 | 151   | Feature release - unified bilingual script (108-key EN/TR table, -Language, bilingual prompt); wrappers removed; gitleaks secret-scan CI; no policy changes; total 151; cumulative chain 24→51→83→123→151 |
| v2.6.2.0 | 2026-09-07 | 151   | Compatibility validation: Brave 1.94.121 (Chromium 152.0.7977.83); no policy changes; total 151; cumulative chain 24›51›83›123›151 |
...
| v2.1.6 | 2026-07-05 | 81    | 15 new Brave-specific policies added, CloudPrintProxyEnabled removed (deprecated) |
...
<a id="tr-v2800-ozet"></a>

### Özet

**Veri katmanı yeniden düzenlemesi.** v2.8.0.0, 151 politika tanımının tamamını betik gövdesinden doğrulanmış, makine tarafından okunabilir bir veri katmanına taşır: `Brave Omega/config.json` (kayıt defteri hedefleri + seviye sırası) ve `Brave Omega/profiles/*.json` (her seviye için bir dosya). `BraveOmega.ps1` artık politikaları çalışma zamanında `Import-OmegaPolicyData` üzerinden tek bir `$OmegaState` içine yükler ve bayt-bayt aynı kayıt defteri çıktısı üretir. Pester paketi, ADMX çapraz referans doğrulayıcı ve politika kataloğu üreticisi aynı veri katmanını okur; böylece derleme, test ve belgeler için tek doğruluk kaynağı hâline gelir. Politika değişikliği yoktur: 5 seviyede toplam **151**; kümülatif zincir 24 → 51 → 83 → 123 → 151 olarak kalır.

| Metrik | Önce (v2.7.3.0) | Sonra (v2.8.0.0) |
|--------|-----------------|-------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 151 | **151** (değişiklik yok) |
| Kümülatif zincir | 24→51→83→123→151 | **24→51→83→123→151** |
| Betik sürümü | v2.7.3.0 | v2.8.0.0 |
| Doğrulanan Brave | 1.95.104 | 1.95.104 |
| Doğrulanan Chromium | 153.0.8010.53 | 153.0.8010.53 |

<a id="tr-v2800-degistirildi"></a>

### Değiştirildi

- **BraveOmega.ps1** - `$ScriptVersion` → `v2.8.0.0`; politika tanımları betikten veri katmanına taşındı; çalışma zamanı politikaları `Import-OmegaPolicyData` üzerinden yükler (tek `$OmegaState`).
- **Veri katmanı** - yeni `Brave Omega/config.json` + `Brave Omega/profiles/<Seviye>.json` (151 politika) tek doğruluk kaynağıdır; ADMX doğrulayıcı, Pester paketi ve katalog üreticisi aynı veriyi okur.
- **Testler ve CI** - TestHelper ve testler veri katmanına bağlandı; `quality.yml` içerisindeki Pester işi artık bilgilendirici bir CodeCoverage raporu da üretir.
- **Sürüm matrisleri ve katalog** - README, SECURITY, index.html, Wiki uyumluluk matrisi ve yeniden üretilmiş `levels.json` güncellendi; `.reg` dosyaları bayt-bayt aynı (politika farkı yok).
...
<a id="tr-v2730-ozet"></a>

### Özet

**Brave 1.95.104 uyumluluk doğrulaması.** v2.7.3.0, yapılandırmayı 18 Eylül 2026'da yayımlanan Brave 1.95.104 (Chromium 153.0.8010.53) sürümüne karşı doğrular. Brave 1.95.102 (Chromium 153.0.8010.48) desteklenmeye devam ediyor. Politika değişikliği yoktur: 5 seviyede toplam **151**; kümülatif zincir 24 → 51 → 83 → 123 → 151 olarak kalır.

| Metrik | Önce (v2.7.2.0) | Sonra (v2.7.3.0) |
|--------|-----------------|-------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 151 | **151** (değişiklik yok) |
| Kümülatif zincir | 24→51→83→123→151 | **24→51→83→123→151** |
| Betik sürümü | v2.7.2.0 | v2.7.3.0 |
| Doğrulanan Brave | 1.95.102 | 1.95.104 |
| Doğrulanan Chromium | 153.0.8010.48 | 153.0.8010.53 |

<a id="tr-v2730-degistirildi"></a>

### Değiştirildi

- **BraveOmega.ps1** — `$ScriptVersion` → `v2.7.3.0`; `$ValidatedBrave` → `1.95.104`; `$ValidatedChromium` → `153`; başlık değişiklik geçmişi ve kanal uyarısı güncellendi.
- **Sürüm matrisleri ve katalog** — README, SECURITY, index.html, Wiki uyumluluk matrisi ve yeniden üretilmiş `levels.json` güncellendi; `.reg` dosyaları bayt-bayt aynı (politika farkı yok).
...
<a id="tr-v2720-ozet"></a>

### Özet

**Brave 1.95.102 uyumluluk doğrulaması.** v2.7.2.0, yapılandırmayı 16 Eylül 2026'da yayımlanan Brave 1.95.102 (Chromium 153.0.8010.48) sürümüne karşı doğrular. Brave 1.95.101 (Chromium 153.0.8010.37) desteklenmeye devam ediyor. Politika değişikliği yoktur: 5 seviyede toplam **151**; kümülatif zincir 24 → 51 → 83 → 123 → 151 olarak kalır.

| Metrik | Önce (v2.7.1.0) | Sonra (v2.7.2.0) |
|--------|-----------------|-------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 151 | **151** (değişiklik yok) |
| Kümülatif zincir | 24→51→83→123→151 | **24→51→83→123→151** |
| Betik sürümü | v2.7.1.0 | v2.7.2.0 |
| Doğrulanan Brave | 1.95.101 | 1.95.102 |
| Doğrulanan Chromium | 153.0.8010.37 | 153.0.8010.48 |

<a id="tr-v2720-degistirildi"></a>

### Değiştirildi

- **BraveOmega.ps1** — `$ScriptVersion` → `v2.7.2.0`; `$ValidatedBrave` → `1.95.102`; `$ValidatedChromium` → `153`; başlık değişiklik geçmişi ve kanal uyarısı güncellendi.
- **Sürüm matrisleri ve katalog** — README, SECURITY, index.html, Wiki uyumluluk matrisi ve yeniden üretilmiş `levels.json` güncellendi; `.reg` dosyaları bayt-bayt aynı (politika farkı yok).
...
<a id="tr-v2710-ozet"></a>

### Özet

**Brave 1.95.101 uyumluluk doğrulaması.** v2.7.1.0, yapılandırmayı 11 Eylül 2026'da yayımlanan Brave 1.95.101 (Chromium 153.0.8010.37) sürümüne karşı doğrular. Brave 1.94.121 (Chromium 152.0.7977.83) desteklenmeye devam ediyor. Politika değişikliği yoktur: 5 seviyede toplam **151**; kümülatif zincir 24 → 51 → 83 → 123 → 151 olarak kalır.

| Metrik | Önce (v2.7.0.0) | Sonra (v2.7.1.0) |
|--------|-----------------|-------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 151 | **151** (değişiklik yok) |
| Kümülatif zincir | 24→51→83→123→151 | **24→51→83→123→151** |
| Betik sürümü | v2.7.0.0 | v2.7.1.0 |
| Doğrulanan Brave | 1.94.121 | 1.95.101 |
| Doğrulanan Chromium | 152.0.7977.83 | 153.0.8010.37 |

<a id="tr-v2710-degistirildi"></a>

### Değiştirildi

- **BraveOmega.ps1** — `$ScriptVersion` → `v2.7.1.0`; `$ValidatedBrave` → `1.95.101`; `$ValidatedChromium` → `153`; başlık değişiklik geçmişi ve kanal uyarısı güncellendi.
- **Sürüm matrisleri ve katalog** — README, SECURITY, index.html, Wiki uyumluluk matrisi ve yeniden üretilmiş `levels.json` güncellendi; `.reg` dosyaları bayt-bayt aynı (politika farkı yok).
...
<a id="tr-v2700-ozet"></a>

### Özet

**Birleşik iki dilli betik.** v2.7.0.0, `BraveOmega-EN.ps1` ve `BraveOmega-TR.ps1` dosyalarını 108 anahtarlı EN/TR metin tablolu tek `BraveOmega.ps1` dosyasında birleştirir: ilk açılışta `Press 1 for English / Türkçe için 2'ye basın` diye sorar; `-Language EN|TR|Auto` ile dil sabitlenebilir. `Brave Omega/` dizininde artık tek betik vardır. Politika değişikliği yoktur: 5 seviyede toplam **151**; kümülatif zincir 24 → 51 → 83 → 123 → 151 olarak kalır. Doğrulanan Brave 1.94.121 (Chromium 152.0.7977.83) olarak kalır.

| Metrik | Önce (v2.6.2.0) | Sonra (v2.7.0.0) |
|--------|-----------------|-------------------|
| Betik dosyası | 2 (EN + TR) | **1** (birleşik iki dilli) |
| Arayüz dili | 2 dosya | **2** (tek metin tablosu) |
| Toplam politika | 151 | **151** (değişiklik yok) |
| Kümülatif zincir | 24→51→83→123→151 | **24→51→83→123→151** |
| Betik sürümü | v2.6.2.0 | v2.7.0.0 |
| Doğrulanan Brave | 1.94.121 | 1.94.121 |
| Doğrulanan Chromium | 152.0.7977.83 | 152.0.7977.83 |

<a id="tr-v2700-eklendi"></a>

### Eklendi

- **Birleşik iki dilli betik** — `BraveOmega.ps1`, tüm arayüz metinlerini `$Strings` tablosunda (108 anahtar, EN + TR) ve `$LevelDisplayNames` eşlemesinde taşır; iki dilli açılış istemli `-Language EN|TR|Auto`; TR kısaltmaları korunur (`-Seviye`, `-Sifirla`, `-SenkronizasyonaIzinVer`, `-Dil`).
- **Gitleaks secret-scan CI** — SHA-256 doğrulamalı sabitlenmiş v8.30.1 ikilisi, push ve pull request'lerde tüm tarihçeyi tarar.

<a id="tr-v2700-degistirildi"></a>

### Değiştirildi

- **`Brave Omega/` altında tek betik** — EN/TR sarmalayıcıları kaldırıldı; testler, ADMX doğrulayıcı, workflow'lar, katalog araçları, README, index.html, Wiki ve şablonlar sarmalayıcısız düzene güncellendi.
- **Gizli TR hatası düzeltildi** — S/MIME kurulum notu Türkçe çalışmalarda hiç görüntülenmiyordu (Türkçe görünen ad İngilizce seviye anahtarlarıyla karşılaştırılıyordu); kanonik `$Level` düzeltir, regresyon testi kapsar.
...
| Metrik | Önce (v2.6.1.1) | Sonra (v2.6.2.0) |
|--------|-----------------|------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 151 | **151** (değişiklik yok) |
| Temel politikaları | 27 | **27** (değişiklik yok) |
| Kümülatif zincir | 24›51›83›123›151 | **24›51›83›123›151** |
| Betik sürümü | v2.6.1.1 | v2.6.2.0 |
| Doğrulanan Brave | 1.94.117 | 1.94.121 |
| Doğrulanan Chromium | 152.0.7977.64 | 152.0.7977.83 |

<a id="tr-v2620-degistirildi"></a>
...
| Metrik | Önce (v2.6.0.0) | Sonra (v2.6.1.0) |
|--------|-----------------|------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 152 | **151** (-1) |
| Temel politikaları | 28 | **27** (-1) |
| Kümülatif zincir | 24›52›84›124›152 | **24›51›83›123›151** |
| Betik sürümü | v2.6.0.0 | v2.6.1.0 |
| Doğrulanan Brave | 1.94.117 | 1.94.117 |
| Doğrulanan Chromium | 152.0.7977.64 | 152.0.7977.64 |

<a id="tr-v2610-kaldirildi"></a>

### Kaldırıldı

- **`DeviceAttributesAllowedForOrigins`** — Temel seviyesinden kaldırıldı. Brave'in Windows sürümünde desteklenmeyen, yalnızca ChromeOS'a özgü Device Attributes API politikası (çalışma zamanında bilinmeyen politika hatası döndürür); Windows'ta sıkılaştırma etkisi yoktur.

<a id="tr-v2610-degistirildi"></a>

### Değiştirildi

- **Seviye sayıları** — Temel 28 › **27**, toplam 152 › **151**; kümülatif zincir artık 24›51›83›123›151 (Brave Yalnız 24, Temel 27, Dengeli 32, Gelişmiş 40, Katı 28).
- **BraveOmega-EN.ps1 / BraveOmega-TR.ps1** — `$ScriptVersion`/`$BetikSurum` › `v2.6.1.0`, kaldırılan politikanın tanım ve sıfırlama listesi girişleri bırakıldı, başlık ve değişiklik geçmişi güncellendi.
- **ADMX doğrulaması** — `$knownAdmxExceptions` boşaltıldı; `admx-validate.ps1`, ADMXCrossReference, PolicyDefinitions ve PolicyMerge testleri 151 politikalık tabana güncellendi.
- **Dokümantasyon ve testler** — README, Wiki, SECURITY.md, index.html ve Pester test paketi 151 politikaya güncellendi.
...
|--------|-----------------|------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 150 | **152** (+2) |
| Gelişmiş eklemeler | 38 | **40** (+2) |
| Kümülatif zincir | 24›52›84›122›150 | **24›52›84›124›152** |
| Betik sürümü | v2.5.5.4 | v2.6.0.0 |
| Doğrulanan Brave | 1.94.117 | 1.94.117 |
| Doğrulanan Chromium | 152.0.7977.64 | 152.0.7977.64 |

<a id="tr-v2600-eklendi"></a>
...
- **Seviye sayıları** — Gelişmiş 38 › **40**, toplam 150 › **152**; kümülatif zincir artık 24›52›84›124›152 (Brave Yalnız 24, Temel 28, Dengeli 32, Gelişmiş 40, Katı 28).
- **BraveOmega-EN.ps1 / BraveOmega-TR.ps1** — `$ScriptVersion`/`$BetikSurum` › `v2.6.0.0`, başlık ve değişiklik geçmişi güncellendi.
- **Dokümantasyon ve testler** — policy-catalog, Brave-Group-Policy-Reference, Wiki, README, SECURITY.md, index.html ve Pester test paketi 152 politikaya güncellendi.
...
| Metrik | Önce (v2.5.5.3) | Sonra (v2.5.5.4) |
|--------|-----------------|------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 150 | 150 |
| Kümülatif zincir | 24›52›84›122›150 | 24›52›84›122›150 |
| Betik sürümü | v2.5.5.3 | v2.5.5.4 |
| Doğrulanan Brave | 1.93.138 | 1.94.117 |
| Doğrulanan Chromium | 151.0.7922.173 | 152.0.7977.64 |

<a id="tr-v2554-degistirildi"></a>

### Değiştirildi

- **Brave sürüm güncellemesi** — Brave **1.94.117** (Chromium 152.0.7977.64) ile doğrulandı. Brave 1.93.138 (Chromium 151.0.7922.173), Brave 1.93.136 (Chromium 151.0.7922.137) ve Brave 1.93.129 (Chromium 151.0.7922.71) desteklenmeye devam eder.
- **BraveOmega-EN.ps1 / BraveOmega-TR.ps1** — `$ScriptVersion`/`$BetikSurum` › `v2.5.5.4`, başlık ve değişiklik geçmişi güncellendi.
- **Dokümantasyon ve testler** — README, SECURITY.md, index.html, Wiki, sürüm notları ve Pester test paketindeki sürüm referansları güncellendi.
...
| Metrik | Önce (v2.5.5.2) | Sonra (v2.5.5.3) |
|--------|-----------------|------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 150 | 150 |
| Kümülatif zincir | 24›52›84›122›150 | 24›52›84›122›150 |
| Betik sürümü | v2.5.5.2 | v2.5.5.3 |
| Doğrulanan Brave | 1.93.136 | 1.93.138 |
| Doğrulanan Chromium | 151.0.7922.137 | 151.0.7922.173 |

<a id="tr-v2553-degistirildi"></a>

### Değiştirildi

- **Brave sürüm güncellemesi** — Brave **1.93.138** (Chromium 151.0.7922.173) ile doğrulandı. Brave 1.93.136 (Chromium 151.0.7922.137) desteklenmeye devam eder.
- **BraveOmega-EN.ps1 / BraveOmega-TR.ps1** — `$ScriptVersion`/`$BetikSurum` › `v2.5.5.3`, başlık ve değişiklik geçmişi güncellendi.
- **Dokümantasyon ve testler** — README, SECURITY.md, index.html, Wiki, sürüm notları ve Pester test paketindeki sürüm referansları güncellendi.
...
| Metrik | Önce (v2.5.5.1) | Sonra (v2.5.5.2) |
|--------|-----------------|------------------|
| Sıkılaştırma seviyesi | 5 | 5 |
| Toplam politika | 150 | 150 |
| Kümülatif zincir | 24›52›84›122›150 | 24›52›84›122›150 |
| Betik sürümü | v2.5.5.1 | v2.5.5.2 |
| Doğrulanan Brave | 1.93.129 | 1.93.136 |
| Doğrulanan Chromium | 151.0.7922.71 | 151.0.7922.137 |

<a id="tr-v2552-duzeltildi"></a>

### Düzeltildi

- **Kritik Sıfırla modu gerilemesi (TR + EN)** — `$HKCU_Hedef`/`$HKLM_Hedef` ve `$HKCU_Target`/`$HKLM_Target` yol sabitleri `-Sıfırla`/`-Reset` bloğundan **sonra** tanımlanıyordu. Sıfırla modu bu nedenle `Test-Path $null` değerlendirdi (Windows PowerShell 5.1 `ParameterBindingValidationException` fırlatır) ve herhangi bir politikayı kaldırmadan çöktü. Tanımlar Sıfırla bloğunun üzerine taşındı; yinelenen geç tanımlar kaldırıldı.
- **ADMX doğrulayıcı sabit kodlanmış eşleme** — `admx/admx-validate.ps1` yalnızca ~80 sabit kodlanmış politika adını doğruluyordu. Artık 150 politikanın tamamını doğrudan `BraveOmega-EN.ps1`'den otomatik keşfeder ve her birini (tür + ValueName) `brave.admx` karşısında doğrular.

<a id="tr-v2552-degistirildi"></a>

### Değiştirildi

- **Brave sürüm güncellemesi** — Brave **1.93.136** (Chromium 151.0.7922.137) ile doğrulandı. Brave 1.93.129 (Chromium 151.0.7922.71) desteklenmeye devam eder.
- **ADMX doğrulayıcı belgelenmiş istisnalar** — `$knownAdmxExceptions`, Brave'in ADMX'inde kasıtlı olarak bulunmayan politikaların ciddi hata yerine uyarı olarak raporlanmasına izin verir (yalnızca `DeviceAttributesAllowedForOrigins` listelenir, CHANGELOG'da belgelenmiştir; yazım hatası susturmak için kullanmayın).
- **BraveOmega-EN.ps1 / BraveOmega-TR.ps1** — `$ScriptVersion`/`$BetikSurum` › `v2.5.5.2`, başlık ve değişiklik geçmişi güncellendi.
- **Testler** — yeni regresyon testleri: Sıfırla modu yol sabiti sıralaması (EN/TR), ADMX doğrulayıcı otomatik keşif sayısı ve belgelenmiş-istisna işleme.
...
|-------|------------|-------------|-------------------|
| v2.8.0.0 | 2026-09-22 | 151   | Veri katmanı yeniden düzenlemesi: politika tanımları config.json + profiles/*.json içine taşındı; betik Import-OmegaPolicyData ile yükler; çalışma zamanı davranış değişmedi; toplam 151; kümülatif zincir 24→51→83→123→151 |
| v2.7.3.0 | 2026-09-19 | 151   | Uyumluluk doğrulaması: Brave 1.95.104 (Chromium 153.0.8010.53); politika değişikliği yok; toplam 151; kümülatif zincir 24→51→83→123→151 |
| v2.7.2.0 | 2026-09-18 | 151   | Uyumluluk doğrulaması: Brave 1.95.102 (Chromium 153.0.8010.48); politika değişikliği yok; toplam 151; kümülatif zincir 24→51→83→123→151 |
| v2.7.1.0 | 2026-09-11 | 151   | Uyumluluk doğrulaması: Brave 1.95.101 (Chromium 153.0.8010.37); politika değişikliği yok; toplam 151; kümülatif zincir 24→51→83→123→151 |
| v2.7.0.0 | 2026-09-09 | 151   | Özellik sürümü - birleşik iki dilli betik (108 anahtarlı EN/TR tablo, -Language, iki dilli istem); sarmalayıcılar kaldırıldı; gitleaks secret-scan CI; politika değişikliği yok; toplam 151; kümülatif zincir 24→51→83→123→151 |
| v2.6.2.0 | 2026-09-07 | 151   | Uyumluluk doğrulaması: Brave 1.94.121 (Chromium 152.0.7977.83); politika değişikliği yok; toplam 151; kümülatif zincir 24›51›83›123›151 |
...
| v2.1.6 | 2026-07-05 | 81    | 15 yeni Brave politikası eklendi, CloudPrintProxyEnabled kaldırıldı (kullanımdan kaldırıldı) |
